...
(function (MD) {
  ...
  // ------------------------------------------------------------- i18n
  const LANGS = [
    { code: "en",    label: "English",            locale: "en-GB", dir: "ltr" },
    { code: "es",    label: "Español",            locale: "es-ES", dir: "ltr" },
    { code: "zh-CN", label: "简体中文 · 普通话",   locale: "zh-CN", dir: "ltr" },
    { code: "zh-HK", label: "廣東話 · 香港",       locale: "zh-HK", dir: "ltr" },
    { code: "zh-TW", label: "繁體中文 · 台灣",     locale: "zh-TW", dir: "ltr" },
    { code: "pl",    label: "Polski",             locale: "pl-PL", dir: "ltr" },
    { code: "ar",    label: "العربية",            locale: "ar",    dir: "rtl" },
    { code: "ro",    label: "Română",             locale: "ro-RO", dir: "ltr" },
  ];
  const DICTS = {};
  const i18n = {
    LANGS, DICTS, lang: "en",
    ...
    async load(code) {
      if (!DICTS[code]) await loadJs(new URL(`i18n/${code}.js`, BASE).href);
      return !!DICTS[code];
    },
  ...
  const skin = {
    ...
    async use(id) {
      const S = MD.skins.SKINS[id] || MD.skins.SKINS.native;
    ...
    classesFor(el) {
      const S = this.current || MD.skins.SKINS.native;
  ...
  class MdSwitcher extends HTMLElement {
    ...
    paint() {
      ...
      if (want.includes("lang")) h += `<label class="md-sel">${ico("globe")}<span class="md-sr">${t("ui.lang", "Language")}</span><select data-lang-set>` +
        LANGS.map(l => `<option value="${l.code}" ${l.code === i18n.lang ? "selected" : ""}>${l.label}</option>`).join("") + "</select></label>";
      ...
      const ls = this.querySelector("[data-lang-set]"); if (ls) ls.onchange = () => i18n.set(ls.value);
  ...
  async function boot(opts = {}) {
    ...
    const prefLang = params.get("lang") || store.get("md.lang") || opts.lang || bestLang();
    const prefSkin = params.get("skin") || store.get("md.skin") || opts.skin || "native";
